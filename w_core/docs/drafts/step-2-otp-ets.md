...
### 2. Telemetry.Server (GenServer de Ingestão)
...
**Interface:**
```elixir
WCore.Telemetry.Server.ingest(node_id, status, payload)
```
...
```elixir
Phoenix.PubSub.broadcast(WCore.PubSub, "user:#{user_id}:metrics", 
  {:metric_ingested, node_id, status, payload})
```

**Subscribers:**
```elixir
Phoenix.PubSub.subscribe(WCore.PubSub, "user:#{current_user.id}:metrics")
```

Isso permite LiveView em tempo real sem pooling.
