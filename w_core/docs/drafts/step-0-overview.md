...
- O dashboard assina tópicos por node no formato `telemetry:node:<node_id>`
- O isolamento multi-tenant é garantido pelo `Scope` (usuário só assina nodes autorizados)
...
{:ecto_sqlite3, "~> 0.10"}        # Persistência
...
3. **Sprint 3:** Write-behind (FlushWorker + batch no SQLite)
4. **Sprint 4:** Dashboard LiveView em tempo real
5. **Sprint 5:** Testes de caos e concorrência
6. **Sprint 6:** Infra, release e Docker
